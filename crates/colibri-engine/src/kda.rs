...
pub fn kda_gate(g: &mut [f32], a_log: &[f32], dt_bias: &[f32], lower_bound: f32, d: &KdaDims) {
    assert_eq!(
        a_log.len(),
        d.dk,
        "K3 A_log is per key-dim, expected {}",
        d.dk
    );
...
mod tests {
    ...
    fn gv() -> Vec<f32> {
        (0..S * H * DK)
            .map(|i| -0.1 - 0.05 * ((i * 2 % 5) as f32))
            .collect()
    }
    fn bv() -> Vec<f32> {
        (0..S * H).map(|i| 0.3 * ((i % 4) as f32 - 1.5)).collect()
    }
    ...
    fn stepwise_equals_batched() {
        ...
        for i in 0..o_all.len() {
            assert!(
                (o_all[i] - o_step[i]).abs() < 1e-6,
                "token-at-a-time diverged at {i}"
            );
        }
    }
    ...
    fn a_log_broadcasts_across_heads_not_dims() {
        ...
        for i in 0..DK {
            assert!(
                (g[i] - g[DK + i]).abs() < 1e-7,
                "key-dim {i} differs between heads: {} vs {}",
                g[i],
                g[DK + i]
            );
        ...
        assert!(
            (g[0] - g[1]).abs() > 1e-3,
            "A_log is not varying across key-dims"
        );
