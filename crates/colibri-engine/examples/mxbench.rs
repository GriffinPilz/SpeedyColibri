...
fn main() {
    ...
    let w = QTensor {
        fmt_code: 6, q4: Bytes::Owned(q4), bs: Bytes::Owned(bs), g: 1.0,
        o: o as i32, i: i as i32, ..Default::default()
    };
    ...
    const N: u32 = 5;
    for _ in 0..N { colibri_engine::matmul_qt(&mut y, &x, &w, 1); }
    let ms = t.elapsed().as_secs_f64() * 1e3 / N as f64;
    println!("[mxbench] {o}x{i}  {ms:.1} ms/call  ({:.0} MMAC/s)  checksum {:.6}",
             (o * i) as f64 / (ms * 1e3), y[0]);
}
